...
| C14 | telegram-bridge | critical | Miry drops pending Telegram Bot API updates before polling. | Source contract | `tests/test-contract.sh` T18 | covered | Cutover must not process stale Telegram updates that accumulated while Miry was offline. |
| C15 | telegram-bridge | critical | Miry records Telegram replies to daily job/gig recommendations as feedback events before Codex routing. | Source contract | `tests/test-contract.sh` T19 | covered | Replying "too ML" to a daily lead must create a feedback event instead of disappearing into a normal Codex task. |
